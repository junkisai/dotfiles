...
モデルと推論設定はユーザー指定・プロジェクト設定・現在の環境を優先する。症状を確認せずに特定モデル向けの値を一律に強制しない。設定を一時変更する場合は既存の設定を引き継いで `--config` に渡す。
...
3. 変更に必要なテスト・lint・型チェックで検証し、今回の変更による失敗を直す。無関係な全検査を毎回繰り返さない
4. 修正内容の要約（指摘への対応に加えて、2で追加した修正も含む）を `<runDir>/changes-round<N>.md` に書き出す
5. コミット・push が依頼に含まれていれば実行する。含まれていなければ修正をローカルに残す。ループを終了するとき（完了・中止のいずれも）、未pushのコミットが残っていれば、その旨（コミット数と内容の要約）とpushするかの確認を最終報告に含める（黙って終えると、PRが未更新のまま完了したと誤解されるため）
...
- レビュー報告に「ビルトインGitHub連携（codex_apps/github_*）の使用を検出」という警告が出た場合は、その内容をユーザーに伝え、この検証器が認識できる証跡の不足を説明する。ユーザーのプラグイン設定は自動変更しない

## git worktree の利用

リポジトリの worktree 規約に従い、start より前に対象 PR の作業環境を用意する。
既存の作業を破棄せず、ループ全体を同じ worktree 内で実行する。
対象ブランチが別の worktree で使われていればその場所を確認し、並行作業を妨げない方法を選ぶ。
...
| サンドボックス内で `Operation not permitted` や外部ネットワークへの接続失敗 | 制限の原因を確認し、実行環境の権限手順で必要なアクセスを得る |
